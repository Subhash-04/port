...
const VARIANTS: Record<string, { activeColor: string | null; gap: number; speed: number; colors: string; noFocus: boolean }> = {
  default: { activeColor: null, gap: 5, speed: 35, colors: '#d9c9b0,#c8b99a,#b0967a', noFocus: false },
  ...
  warm: { activeColor: '#f0a36c', gap: 6, speed: 30, colors: '#f4f1ea,#f0a36c,#c64f17', noFocus: false },
};
...
export default function PixelCard({ variant = 'default', gap, speed, colors, noFocus, className = '', style, children }: PixelCardProps) {
  ...
  return (
    <div
      ...
      onFocus={finalNoFocus ? undefined : (e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) handleAnimation('appear'); }}
      onBlur={finalNoFocus ? undefined : (e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) handleAnimation('disappear'); }}
