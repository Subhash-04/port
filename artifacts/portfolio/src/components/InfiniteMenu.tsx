...
function createProgram(gl: WebGL2RenderingContext, shaderSources: string[], tfVaryings: string[] | null, attribLocations: Record<string, number> | null) {
...
class ArcballControl {
  ...
  snapTargetDirection?: Float32Array;
  EPSILON = 0.1;
  ...
  update(deltaTime: number, targetFrameDuration = 16) {
    ...
    if (this.isPointerDown) {
      ...
      if (this.snapTargetDirection) {
        ...
        const a = this.snapTargetDirection; const b = this.snapDirection;
  ...
  quatFromVectors(a: Float32Array, b: Float32Array, out: Float32Array, angleFactor = 1) {
  ...
  project(pos: Float32Array) {
    ...
    let z = 0;
    const xySq = x * x + y * y; const rSq = r * r;
...
class InfiniteGridMenu {
  ...
  discLocations!: Record<string, WebGLUniformLocation | number | null>;
  discGeo!: DiscGeometry;
  discBuffers!: ReturnType<Geometry['data']['__proto__']['constructor']>;
  ...
  init(onInit: ((sk: InfiniteGridMenu) => void) | null) {
    ...
    ], this.discBuffers.indices as unknown as number[]);
  ...
  initTexture() {
    ...
    Promise.all(this.items.map(item => new Promise<HTMLImageElement>(resolve => {
      ...
      images.forEach((img, i) => {
        ...
        try { ctx.drawImage(img, x, y, cellSize, cellSize); } catch (e) { /* ignore */ }
      });
  ...
  initDiscInstances(count: number) {
    ...
    const mat4AttribSlotCount = 4; const bytesPerMatrix = 16 * 4;
  ...
  render() {
    ...
    gl.clearColor(0.957, 0.949, 0.918, 1);
    ...
    gl.uniform4f(this.discLocations.uRotationAxisVelocity as WebGLUniformLocation, this.control.rotationAxis[0], this.control.rotationAxis[1], this.control.rotationAxis[2], this.smoothRotationVelocity * 1.1);
  ...
  findNearestVertexIndex() {
    ...
    let maxD = -1; let nearestVertexIndex = 0;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items?: InfiniteMenuItem[]; scale?: number }) {
  ...
  useEffect(() => {
    ...
    // Pre-check WebGL2 support
    const testCtx = canvas.getContext('webgl2');
    if (!testCtx) { setWebglFailed(true); return; }

    const handleActiveItem = (index: number) => { setActiveItem(items[index % items.length]); };
